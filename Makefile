...
cc:
	@echo "Cross Complie"
	set GOOS=darwin& set GOARCH=386& go build -o bin/gpc-darwin-386 cmd/gpc.go
	set GOOS=darwin& set GOARCH=arm64& go build -o bin/gpc-darwin-arm64 cmd/gpc.go
	set GOOS=darwin& set GOARCH=arm& go build -o bin/gpc-darwin-arm cmd/gpc.go
	set GOOS=linux& set GOARCH=arm64& go build -o bin/gpc-linux-amd64 cmd/gpc.go
	set GOOS=linux& set GOARCH=arm& go build -o bin/gpc-linux-amd64 cmd/gpc.go
	set GOOS=windows& set GOARCH=arm64& go build -o bin/gpc-windows-arm.exe cmd/gpc.go
	set GOOS=windows& set GOARCH=arm& go build -o bin/gpc-windows-arm.exe cmd/gpc.go
